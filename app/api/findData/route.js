...
async function handler(req) {
    if (req.method === "POST") {
        ...
        try {
            ...
            console.error("Error connecting to the database in findData.js:", error);
            return res
                .status(500)
                .json({ message: "Error connecting to the database in findData.js" });
        }
        ...
        try {
            ...
            if (!user) {
                // User not found
                return res.status(404).json({ message: "User not found in findData.js" });
            }
            ...
            return res.status(500).json({
                message: `Error fetching user data in findData.js: ${error.message}`,
            });
        } finally {
            if (client) {
                await client.close();
